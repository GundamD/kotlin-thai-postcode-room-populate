...
import * as addressDb from "./addressDb"
import { duration } from "moment"

/**
 * Address file
 */
const FILE_ADDRESS = "raw_database.json"

/**
 * Takes address list source files and populates SQLite database
...
export async function addresses(
  ...
  await populate(Path.join(schemaPath, schemaFile), db, async function (
    this: Database
  ): Promise<void> {
    await addressDb.populate(
      db,
      await readJsonData<Array<ThaiAddress>>(Path.join(dataDir, FILE_ADDRESS))
    )
  })
...
async function findLatestSchema(schemaPath: string): Promise<string> {
  return new Promise<string>(function (resolve, reject) {
    ...
    fs.readdir(schemaPath, { encoding: "utf8", withFileTypes: true }, function (
      err,
      files
    ) {
      if (err) {
